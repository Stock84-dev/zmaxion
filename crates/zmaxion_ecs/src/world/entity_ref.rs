...
impl<'w> EntityRef<'w> {
    ...
    /// - The returned reference must not be used after this component is moved which may happen
    ///   from **any** `insert_component`, `remove_component` or `despawn` operation on this world
    ///   (non-exhaustive list).
    #[inline]
    pub unsafe fn get_unchecked_mut<T: Component>(
        ...
        unimplemented!()
        //        get_component_and_ticks_with_type(self.world, TypeId::of::<T>(), self.entity,
        // self.location)            .map(|(value, ticks)| Mut {
...
/// Get a raw pointer to a particular [`Component`] on a particular [`Entity`] in the provided
/// [`World`].
///
/// # Safety
...
// TODO: move to Storages?
/// Get a raw pointer to the [`ComponentTicks`] of a particular [`Component`] on a particular
/// [`Entity`] in the provided [World].
///
/// # Safety
...
/// Get a raw pointer to a particular [`Component`] by [`TypeId`] on a particular [`Entity`] in the
/// provided [`World`].
///
/// # Safety
...
/// Get a raw pointer to the [`ComponentTicks`] of a particular [`Component`] by [`TypeId`] on a
/// particular [`Entity`] in the provided [`World`].
///
/// # Safety
/// `entity_location` must be within bounds of an archetype that exists.
...
mod tests {
    ...
    use crate::{component::ComponentId, prelude::*}; // for the `#[derive(Component)]`
