    ...
    I::Item: Borrow<Entity>,
{
    ...
    unsafe fn fetch_next_aliased_unchecked(&mut self) -> Option<QueryItem<'w, Q>> {
        for entity in self.entity_iter.by_ref() {
            ...
            // `location.index` is an archetype index row in range of the current archetype, because
            // if it was not, the match above would have `continue`d
            if self.filter.archetype_filter_fetch(location.index) {
                // SAFETY: set_archetype was called prior, `location.index` is an archetype index in
                // range of the current archetype
...
struct QueryIterationCursor<'w, 's, Q: WorldQuery, F: WorldQuery> {
    ...
    fetch: QueryFetch<'w, Q>,
    filter: QueryFetch<'w, F>,
    // length of the table table or length of the archetype, depending on whether both `Q`'s and
    // `F`'s fetches are dense
    current_len: usize,
    // either table row or archetype index, depending on whether both `Q`'s and `F`'s fetches are
    // dense
    current_index: usize,
...
impl<'w, 's, Q: WorldQuery, F: WorldQuery> QueryIterationCursor<'w, 's, Q, F> {
    ...
    // QueryState::for_each_unchecked_manual, QueryState::par_for_each_unchecked_manual
    /// # Safety
    ...
    unsafe fn next(
        ...
        if Self::IS_DENSE {
            loop {
                ...
                // `current_index` is a table row in range of the current table, because if it was
            ...
            loop {
                ...
                // SAFETY: set_archetype was called prior.
                // `current_index` is an archetype index row in range of the current archetype,
                ...
                // SAFETY: set_archetype was called prior, `current_index` is an archetype index in
                // range of the current archetype `current_index` is an archetype
                // index row in range of the current archetype, because if it was not, then the if
                // above would have been executed.
