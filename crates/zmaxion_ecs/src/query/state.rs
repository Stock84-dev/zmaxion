...
/// Provides scoped access to a [`World`] state according to a given [`WorldQuery`] and query
/// filter.
#[repr(C)]
// SAFETY NOTE:
...
impl<Q: WorldQuery, F: WorldQuery> QueryState<Q, F> {
    ...
    pub fn as_readonly(&self) -> &QueryState<Q::ReadOnly, F::ReadOnly> {
        ...
        // have a subset of the access, and match the exact same archetypes/tables as `Q`/`F`
        // respectively.
    ...
    /// # SAFETY
    ///
    /// `NewQ` must have a subset of the access that `Q` does and match the exact same
...
impl<Q: WorldQuery, F: WorldQuery> QueryState<Q, F> {
    ...
    /// Takes a query for the given [`World`], checks if the given world is the same as the query,
    /// and generates new archetypes for the given world.
    ///
    /// # Panics
    ///
    ...
    pub fn validate_world(&self, world: &World) {
        assert!(
            ...
            "Attempted to use {} with a mismatched World. QueryStates can only be used with the \
             World they were created from.",
...
mod tests {
    ...
    fn get_many_unchecked_manual_uniqueness() {
        ...
        // SAFETY: mutable access is not checked, but we own the world and don't use the query
        // results
        assert!(unsafe {
            query_state
