...
impl World {
    ...
    /// Returns the components of an [`Entity`](crate::entity::Entity) through
    ...
    /// Returns an [`EntityMut`] for the given `entity` (if it exists) or spawns one if it doesn't
    /// exist. This will return [`None`] if the `entity` exists with a different generation.
    ///
    /// # Note
    ...
    /// let a = world
    ///     .spawn()
    ///     .insert_bundle((Order(2), Label("second")))
    ///     .id();
    ...
    pub fn non_send_resource<R: 'static>(&self) -> &R {
        match self.get_non_send_resource() {
            ...
            None => panic!(
                ...
                Did you forget to add it using `app.insert_non_send_resource` / \
    ...
    /// ensure that there is either only one mutable access or multiple immutable accesses at a
    /// time.
    #[inline]
    ...
    /// bundle (if the entity does not exist), or inserts the [Bundle] (if the entity already
    /// exists). This is faster than doing equivalent operations one-by-one.
    ...
    //    #[inline]
    //    pub fn send_event_batch<E: crate::event::Event>(&mut self, events: impl Iterator<Item =
    // E>) {        match self.get_resource_mut::<crate::event::Events<E>>() {
...
impl World {
    ...
    /// use this in cases where the actual types are not known at compile time.**
    //    #[inline]
    //    pub fn get_resource_mut_by_id(&mut self, component_id: ComponentId) ->
    // Option<MutUntyped<'_>> {        let info = self.components.get_info(component_id)?;
    //        if !info.is_send_and_sync() {
    //            self.validate_non_send_access_untyped(info.name());
    //        }
    //
    //        let column = self.get_populated_resource_column(component_id)?;
    //
    //        // SAFETY: get_data_ptr requires that the mutability rules are not violated, and the
    // caller promises        // to only modify the resource while the mutable borrow of the
    // world is valid        let ticks = Ticks {
    //            // SAFETY:
    ...
    //
    //        Some(MutUntyped {
    //            // SAFETY: world access is unique, so no other reference can exist at the same
    // time            value: unsafe { column.get_data_ptr().assert_unique() },
    //            ticks,
    //        })
    //    }
    ...
    /// [`ComponentId`]. Returns [None] if the `entity` does not have a [Component] of the given
    /// type.
    ///
...
mod tests {
    ...
    #[test]
    #[should_panic = "insert_resource_by_id called with component id which doesn't exist in this \
                      world"]
    fn insert_resource_by_id_invalid_component_id() {
        ...
        let mut world = World::new();
        OwningPtr::make((), |ptr| {
            // SAFETY: ptr must be valid for the component_id `invalid_component_id` which is
            // invalid, but checked by `insert_resource_by_id`
            unsafe {
