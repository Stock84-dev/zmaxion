...
pub mod prelude {
    ...
    pub use crate::{
        ...
        //        schedule::{
        //            AmbiguitySetLabel, ExclusiveSystemDescriptorCoercion,
        // ParallelSystemDescriptorCoercion,            RunCriteria,
        // RunCriteriaDescriptorCoercion, RunCriteriaLabel, Schedule, Stage,
...
//            vec![e3, e1],
//            "changed entities list should not change (although the order will due to archetype \
//             moves)"
//        );
//
...
//        assert!(
//            world_b
//                .get_entity(high_non_existent_but_reserved_entity)
//                .is_none(),
//            "entities between high-newly allocated entity and continuous block of existing \
    ...
    fn a() {
        ...
        rt.block_on(async {
            ...
            while now.elapsed().as_secs() < 2 {
                ...
                let a = stream.next().now_or_never();
                match a {
                    Some(Some(output)) => {}
                    _ => {
